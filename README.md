...
Besides the definition of parameters for the feature calculation, there is another purpose for some of the classes at [testFeature](https://github.com/pedroig/Parkinsons-Disease-Digital-Biomarker/blob/master/Visualization/testFeature.py): data analysis from the visualization of the raw data. To fulfill this purpose, there is the class Overview that generates a grid of plots for all the axes of the accelerometer data and also for the magnitude of the resultant acceleration vector. The Overview class also provides an option to visualize the segments of each step in the graph plotted by making use of the pedometer data, but it lacks precision due to the low quality of the pedometer data.
