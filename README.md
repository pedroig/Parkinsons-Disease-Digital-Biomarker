...
A second step of data cleaning to be performed in the splitting procedure aiming to solve the skewed distribution of samples per healthCode in the dataset was also considered. There are people that recorded up to 450 samples of data while the average number of samples per healthCode is around 11. This asymmetrical distribution could have negative effects on the training data, possibly causing a slight overfitting for a specific healthcode which would be especially bad if the healthCode corresponds to an outlier. The solution considered to solve this issue was to limit the number of samples per person, using only the first 10 occurrences per healthCode. However, this solution would imply a huge loss in the dataset, so it was disconsidered.
